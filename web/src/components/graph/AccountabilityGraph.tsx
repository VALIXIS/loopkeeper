    ...
    <div className="space-y-6 pb-12 animate-fade-in-up">
      ...
      <div className="rounded-3xl bg-gradient-to-br from-indigo-950/60 via-slate-900/90 to-slate-950 border border-indigo-500/40 p-6 sm:p-8 shadow-2xl space-y-4 backdrop-blur-xl relative overflow-hidden">
        ...
        <div className="pt-4 border-t border-white/[0.08] flex flex-wrap items-center justify-between gap-4 text-xs relative z-10">
          ...
          <div className="flex items-center gap-2">
            <span className="text-slate-400 text-xs">Filter Assignee:</span>
            <select
              value={filterOwner}
              onChange={e => setFilterOwner(e.target.value)}
              className="px-3 py-1.5 rounded-xl bg-slate-950 border border-white/[0.1] text-xs text-slate-200 focus:outline-none focus:border-indigo-500 font-sans"
            >
      ...
      <div
        className="rounded-3xl bg-slate-950/90 border border-white/[0.08] p-6 overflow-x-auto shadow-2xl relative backdrop-blur-xl transition-transform duration-300"
        style={{ transform: `scale(${zoomLevel})`, transformOrigin: 'top left' }}
      >
        {/* Step Columns Headers */}
        <div className="min-w-[1000px] grid grid-cols-6 gap-4 pb-4 border-b border-white/[0.08] text-[11px] font-bold uppercase tracking-wider text-slate-400 text-center font-mono">
            ...
            return (
              <div
                ...
                className={`group cursor-pointer rounded-2xl p-3.5 border transition-all duration-300 grid grid-cols-6 gap-3 items-center relative ${
                  isSelected
                    ? 'bg-gradient-to-r from-indigo-950/70 via-slate-900/90 to-slate-900 border-cyan-400/90 ring-2 ring-cyan-400/40 shadow-xl shadow-cyan-500/15 scale-[1.01]'
                    : 'bg-slate-900/60 border-white/[0.06] hover:bg-slate-900 hover:border-slate-700'
                }`}
              >
                {/* 1. Meeting Node */}
                <div className="p-3 rounded-xl bg-slate-950/90 border border-indigo-500/30 text-left space-y-1 shadow-inner">
                  <span className="text-[10px] font-mono text-indigo-400 font-bold block truncate">
                    {origin?.source.toUpperCase() || 'MEETING'}
                  </span>
                  <div className="text-xs font-bold text-slate-200 truncate" title={origin?.title}>
                ...
                {/* 2. Commitment Node */}
                <div className="p-3 rounded-xl bg-slate-950/90 border border-cyan-500/30 text-left space-y-1 shadow-inner">
                  <div className="text-xs font-bold text-slate-100 line-clamp-2" title={item.title}>
                    {item.title}
                  </div>
                ...
                {/* 3. Owner Node */}
                <div className="p-3 rounded-xl bg-slate-950/90 border border-purple-500/30 text-center space-y-1 shadow-inner">
                ...
                {/* 4. Deadline Node */}
                <div className="p-3 rounded-xl bg-slate-950/90 border border-amber-500/30 text-center space-y-1 shadow-inner">
                ...
                <div className="p-3 rounded-xl bg-slate-950/90 border border-white/[0.08] text-center space-y-1 shadow-inner">
                  {hasPostponement ? (
                    <div className="space-y-0.5">
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-rose-500/20 text-rose-300 border border-rose-500/30 block font-mono">
                        {item.postponement_count}x Postponed
                      </span>
                      <span className="text-[10px] text-slate-400 block">Deadline Drifted</span>
                    </div>
                  ) : (
                    <div className="space-y-0.5">
                      <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 block font-mono">
                        No Slippage
                      </span>
                      <span className="text-[10px] text-slate-400 block">On Schedule</span>
                    </div>
                  )}
                </div>
      ...
      {selectedItem && (
        <div className="p-6 rounded-3xl bg-slate-900/90 border border-white/[0.08] space-y-4 shadow-2xl backdrop-blur-xl animate-fade-in-up">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs">
            <div className="p-4 rounded-2xl bg-slate-950 border border-white/[0.06]">
              <span className="text-slate-400 block font-semibold text-[11px]">Originating Meeting</span>
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-white/[0.06]">
              <span className="text-slate-400 block font-semibold text-[11px]">Responsible Owner</span>
              <span className="text-slate-200 font-bold block mt-1">
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-white/[0.06]">
              <span className="text-slate-400 block font-semibold text-[11px]">Deduplication Matching</span>
              <span className="text-cyan-400 font-bold block mt-1 font-mono">
                {selectedItem.match_reason || 'Identified as new commitment'}
              </span>
            </div>

            <div className="p-4 rounded-2xl bg-slate-950 border border-white/[0.06]">
              <span className="text-slate-400 block font-semibold text-[11px]">Postponement History</span>
              <span
